...
export default {
  ...
  computed: {
    ...
    applyFields() {
      document.querySelector('table').ad
    },
    ...
    itemProvider() {
      ...
      this.staff_res = 0 // parttime + fulltime

      this.filtered.forEach((xx,index) => {
          if(xx.full_time == 1) {
            this.staff_res += 1;
          } else {
            this.staff_res += 0.5;
  ...
  methods: {
    ...
    getUsers() {
      ...
      axios.post('/timetracking/get-persons', filter)
        .then(response => {
