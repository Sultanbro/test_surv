...
export default {
	...
	methods: {
		...
		getPlan(item){
			...
			const dailyPlan = Number(item.histories_latest?.payload?.daily_plan || item.activity?.daily_plan || item.daily_plan || 0)
			if(!dailyPlan) return item.histories_latest?.payload?.plan || item.plan
