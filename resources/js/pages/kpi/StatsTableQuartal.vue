    ...
    <table class="table j-table table-bordered table-sm mb-3 collapse-table">
            ...
            <tr>
                ...
                <td class="text-left">
                    <div class="d-flex aic p-1">
                        <i class="fa fa-user ml-2 color-user"></i> 
                    ...
                    <td colspan="2">
                        <table class="table b-table table-bordered table-sm table-responsive mb-0 table-inner">
                            ...
                            <tr>
                                ...
                                <td v-if="page_item.items.plan <= page_item.items.fact">Выполнено</td>
                                <td v-else>Не выполнено</td>
                                <td v-if="page_item.items.plan <= page_item.items.fact">{{page_item.items.sum}}</td>
                                <td v-else>0</td>
                            </tr>
                        </table>
                    </td>
        ...
        <template v-for="(page_item, p) in groups" v-if="page_item[0].name.includes(searchText) || searchText.length == 0">
            <tr>
                ...
                <td class="text-left">
                    <div class="d-flex aic p-1">
                        <i class="fa fa-users ml-2 color-group"></i> 
            ...
            <template v-if="page_item.expanded">
                <tr>
                    <td colspan="2">
                        <table class="table b-table table-bordered table-sm table-responsive mb-0 table-inner" v-for="(user, i) in page_item" v-if="i != 'expanded'">
                            <tr>
                               <th @click="expand_group_user(i, p)"
                                    class="pointer b-table-sticky-column">
                                    <div class="d-flex px-2">
                                        <i class="fa fa-minus mt-1" v-if="user.expended"></i>
                                        <i class="fa fa-plus mt-1" v-else></i>
                                        <span class="ml-2">{{ parseInt(i) + 1 }}</span>
                                    </div>
                               </th>
                                <th colspan="9">{{user.full_name}}</th> 
                            </tr>
