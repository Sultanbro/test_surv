...
<div class="mt-5 quality">
    <div class="row">
        ...
        <div class="col-3 d-flex align-items-start">
            ...
            <button class="btn btn-success rounded change-type" @click="change_type()" v-if="Number(activeuserid) == 18 || Number(activeuserid) == 5 || Number(activeuserid) == 157">
                <span v-if="can_add_records">Оценка с U-calls</span> 
                <span v-else>Ручная оценка</span> 
            </button>
            <button class="btn btn-success rounded change-type ml-2" 
                @click="showCritWindow = true" 
                v-if="can_add_records && [5,18,157].includes(Number(activeuserid))">
                <span>Критерии</span> 
            </button>
        </div>
        <div class="col-2" v-if="Number(activeuserid) == 18 || Number(activeuserid) == 5 || Number(activeuserid) == 157">
    ...
    <div v-if="this.hasPermission">
        <a-tabs type="card" v-if="dataLoaded">
                    ...
                    <table class="table b-table table-bordered table-sm">
                        ...
                        <tr v-for="(item, index) in items" :key="index"> 
                            <td class="b-table-sticky-column text-left t-name wd">
                                <div>{{ item.name }}
                                    <b-badge variant="success" v-if="item.groupName == 'Просрочники'">{{ item.groupName }}</b-badge>
                                    <b-badge variant="primary" v-else>{{ item.groupName }}</b-badge>
                                </div> 
                            ...
                            <template v-for="(field, key) in fields" > 
                                <td :class="field.klass"
                                    :key="key">
                                    <input v-if="field.type == 'day' && can_add_records != true" 
                                        type="number"
                                        ...
                                        <div v-else>
                                            <div v-if="item.weeks[field.key] != 0">{{ item.weeks[field.key] }}</div>
                                        </div>
                                </td>
                            </template>
                        </tr>
            ...
            <a-tab-pane tab="Оценка переговоров" key="3" @change="changeTab" v-if="can_add_records">

                

                <div class="row">
                    <div class="col-6 col-md-3">
                        <select class="form-control" v-model="filters.currentEmployee" @change="filterRecords">
                            <option :value="0">Выберите сотрудника</option>
                            <option v-for="item in items" :value="item.id" :key="item.id">{{item.name}}</option>
                        </select>
                    </div>
                    <div class="col-2 col-md-1 d-flex align-items-center">
                        <select class="form-control" v-model="currentDay" @change="fetchData">
                            <option value="0">Все дни</option>
                            <option v-for="day in this.monthInfo.daysInMonth" :value="day" :key="day">{{ day }}</option>
                        </select>   
                    </div>
                    <div class="col-4 col-md-8 d-flex align-items-center">
                        ...
                        <b-button variant="success" @click="exportData()" class="mr-1">
                    ...
                    <div class="col-12 col-md-12 d-flex mt-2 mb-2">
                        <p class="mb-0">Найдено записей: <b class="bluish">{{ records.total }}</b></p>
                        <p class="ml-3 mb-0" v-if="records_unique != 0">Кол-во сотрудников: <b class="bluish">{{ records_unique }}</b></p>
                        <p class="ml-3 mb-0" v-if="avgMonth != 0">Среднее за месяц: <b class="bluish">{{ avgMonth }}</b></p>
                        <p class="ml-3 mb-0" v-if="avgDay != 0">Среднее за день: <b class="bluish">{{ avgDay }}</b></p>
                    </div>
                </div>
                ...
                <div class="table-responsive my-table">
                    <table class="table b-table table-sm  table-bordered records-table">
                        ...
                        <tr :class="{'selected' : record.editable, 'changed' : record.changed}">
                            <td class="b-table-sticky-column text-left t-name wd">
                            ...
                            <template v-if="currentGroup == 42">
                                <td class="text-left segment-width" v-if="record.editable">
                                    <div>
                                        <select v-model="record.segment_id" class="form-control text-center sg" @change="statusChanged(record)">
                                            <option :value="index" v-for="(segm, index) in segment">{{ segm }}</option>
                                        </select>
                                    </div>
                                </td>
                                <td v-else class="text-center segment-width" @click="editMode(record)">
                                    <div >
                            ...
                            <td class="text-center phoner" v-if="record.editable">
                                <div>
                                    <input type="text" v-model="record.phone" class="form-control text-center" @focus="$event.target.select()" @change="statusChanged(record)">
                                </div>
                            </td>
                            <td class="text-center phoner" v-else @click="editMode(record)">
                                <div >
                                    {{ record.phone }}
                                </div>
                            </td>
                            ...
                            <template v-if="currentGroup == 42">
                                <td class="text-center" v-if="record.editable">
                                    <div >
                                        <input type="text" v-model="record.dayOfDelay" class="form-control text-center" @focus="$event.target.select()" @change="statusChanged(record)">
                                    </div>
                                </td>
                            ...
                            <td class="text-center" v-if="record.editable">
                                <div>
                                    <input type="text" v-model="record.interlocutor" class="form-control text-center" @focus="$event.target.select()" @change="statusChanged(record)">
                                </div>
                            </td>
                            <td class="text-center" v-else @click="editMode(record)">
                                <div  >
                            ...
                            <td class="text-center"  v-if="record.editable">
                                <div>
                                    <input type="date" v-model="record.date" class="form-control text-center" placeholder="dd-mm-yyyy" min="1997-01-01" max="2030-12-31" @change="statusChanged(record)">
                                </div>
                            </td>
                            <td class="text-center" v-else @click="editMode(record)">
                                <div  >
                            ...
                            <template v-for="(param, pk) in params">
                               <td class="text-center params" v-if="record.editable">
                                ...
                                <td class="text-center params" v-else @click="editMode(record)">
                                    <div>
                                        {{ record['param' + pk] }}
                                    </div>
                                </td>
                            ...
                            <td class="text-left" v-if="record.editable">
                                <div >
                                    <input type="text" v-model="record.comments" class="form-control" @focus="$event.target.select()" @change="statusChanged(record)">
                                </div>
                            </td>
                            ...
                            <td class="actions" @click="editMode(record)">
                                <div>
                                    ...
                                    <b-button v-if="record.editable" variant="success" size="sm" @click="saveRecord(record)">
                            ...
                            <td class="actions" @click="editMode(record)"> 
                                <div>
                                    <b-button variant="danger" size="sm" @click="deleteRecordModal(record, index)">
                ...
                <div>
                  <pagination :data="records" @pagination-change-page="getResults" :limit="3"></pagination>
    ...
    <b-modal id="delete-modal" hide-footer>
            ...
            <div class="">
                ...
                <div class="d-flex">
                    <b-button class="mt-3 mr-1" variant="danger" block @click="deleteRecord">Удалить</b-button>
                    <b-button variant="primary" class="mt-3 ml-1" block @click="$bvModal.hide('delete-modal')">Отмена</b-button>
                </div>
            </div>
    ...
    <a-modal v-model="showCritWindow"  title="Критерии оценки" @ok="showCritWindow = false" :width="400" class="modalle">
         ...
         <div class="row">
            <div class="col-12 d-flex mb-1" v-for="crit in params">
                ...
                <input type="text" v-model="crit.name" class="form-control form-control-sm">
            </div>

            <div class="col-12">
                <button class="btn btn-sm btn-primary rounded" @click="addParam()">Добавить</button>
                <button class="btn btn-sm btn-success rounded" @click="saveParam()">Сохранить</button>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        saveParam() {
            ...
            axios.post("/timetracking/quality-control/crits/save", {
                    'crits': this.params,
        ...
        addRecord() {
            if(this.filters.currentEmployee == 0) return this.$message.info('Выберите сотрудника!')
        ...
        deleteRecord() {
            ...
            axios 
                ...
                .then((response) => {
                    ...
                    // ES6 Func 
                    let index = this.records.data.findIndex(x => x.id === this.newRecord.id);
        ...
        change_type() {
            ...
            if(e) {
                let loader = this.$loading.show();
                axios.post("/timetracking/quality-control/change-type", {
...
.table {
    ...
    &.b-table.table-sm>thead>tr>[aria-sort]:not(.b-table-sort-icon-left),
    &.b-table.table-sm>tfoot>tr>[aria-sort]:not(.b-table-sort-icon-left) {
...
.records-table select.form-control:not([size]):not([multiple]).form-control,
.records-table input.form-control {
    padding: 2px;
    height: auto;
    font-size: 0.8rem;
    text-align: left;
...
.records-table .number input.form-control {
    padding-left: 14px;
}
