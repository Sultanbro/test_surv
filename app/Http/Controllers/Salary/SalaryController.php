...
class SalaryController extends Controller
{
    ...
    public function salaries(Request $request)
    {
        ...
        $groups = ProfileGroup::query()->where('active', true)
            ->orWhere(fn ($query) => $query->where('active', false)->where(
                fn ($query) => $query->whereNotNull('archived_date')->where(fn($query) =>
                $query->whereDate('archived_date', '>=', $date->format('Y-m-d')))))
            ->get();
    ...
    public function test(Request $request) {
        $timers = [];
        $timers['all'] = microtime(true);
        $timers['getGroups'] = microtime(true);
        ...
        // 42, 48, 53, 71, 88, 96, 97
        // 71f, 88f, 97f
        $groups = ProfileGroup::where('id', 42)
            ->with('workChart')
            ->get();
        $timers['getGroups'] = microtime(true) - $timers['getGroups'];
        ...
        $timers['allGroups'] = microtime(true);
        $workingGroups = Salary::getAllTotals($request->date, $groups, 1);
        $firedGroups = Salary::getAllTotals($request->date, $groups, 2);
        $timers['allGroups'] = microtime(true) - $timers['allGroups'];
        dump('-----------------------');

        $timers['old'] = microtime(true);
        $oldWorkingGroupds = [];
        $oldFiredGroupds = [];
        foreach ($groups as $key => $group) {
            // $timers[$group->name] = microtime(true);

            $salary_working = Salary::getTotal($request->date, $group->id, 1);
            $salary_fired = Salary::getTotal($request->date, $group->id, 2);

            $oldWorkingGroupds[$group->id] = $salary_working;
            $oldFiredGroupds[$group->id] = $salary_fired;

            // // array_push($group_ids, $group->id);
            // $date = Carbon::parse($request->date)->day(1)->format('Y-m-d');
            ...
            //      ]);
            //  }
            // $timers[$group->name] = microtime(true) - $timers[$group->name];
        }
        $timers['old'] = microtime(true) - $timers['old'];

        $timers['all'] = microtime(true) - $timers['all'];
        ...
        //     'oldWorkingGroupds' => $oldWorkingGroupds,
        //     'firedGroups' => $firedGroups,
        //     'oldFiredGroupds' => $oldFiredGroupds,
        // ]);
