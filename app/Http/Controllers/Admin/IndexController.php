...
/*
    ...
    public function userroles()
    {
      ...
      return view('admin.roleslist')->with('users', $users);

    }

    public function userrolesUpdate(Request $request, $domain, $tld, $id)
    {

        $user = User::find($id);
        $books = BookCategory::where('parent_cat_id', NULL)->where('is_deleted', 0)->get();

        if($request->isMethod('post')) {

          $data = ["page1" => $request->str1,
            "page2" => $request->str2,
            "page3" => $request->str3,
            "page4" => $request->str4,
            "page5" => $request->str5,
            ...
            "page8" => $request->str8,
            "page9" => $request->str9,
            "page10" => $request->str10,
            "page11" => $request->str11,
            "page12" => $request->str12,
            "page13" => $request->str13,
            "page14" => $request->str14,
            "page15" => $request->str15,
            "page16" => $request->str16,
            "page17" => $request->str17,
            "page18" => $request->str18,
            "page19" => $request->str19,
            "page20" => $request->str20,
            "page21" => $request->str21,
            "page22" => $request->str22,
            "persons" => $request->persons,
          ];


          if ($request->admin == 'on') { $admin = 1; } else { $admin = 0; };

          $user->UF_ADMIN = $admin;
          $user->roles = $data;
          $user->books = $request->books_ids;
          $user->save();

        }

        return view('admin.roles')->with('title', 'Права пользователей')->with('user', $user)->with('books', $books);
    ...
    public function passwords(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            if ($request['id'] !== '') {
                $access = BpartnersBooksPass::find($request['id']);
                    $access->login = $request['login'];
                    $access->password = $request['password'];
                    $access->page     = $request['page'];
                    $access->save();
            } else {
                DB::connection('bpartners_db')->table('passwords')->insert([
                    'login' => $request['login'],
