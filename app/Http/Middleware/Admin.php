...
class Admin {
    ...
    public function handle($request, Closure $next) {
        ...
        Auth::user()->can = [
            ...
            'hr_view' => $user->is_admin == 1 || in_array($user->id, [5263,9974,10230]),
            'analytics_view' => $user->is_admin == 1 || in_array($user->id, [5,11533,5494,12787,13583,3782,4593,3794,3423,3865,3957,1739,2966,13337,3423,13583]),
            'tabel_view' => $user->is_admin == 1 || in_array($user->id, [5263,9974,11533,5494,12787,1358, 3460, 1739,2966,13337,3794,3423,4143]),
            'entertime_view' => $user->is_admin == 1 || in_array($user->id, [5263,9974,11533,5494,12787,13583, 1739,2966,3794,3423,4143]),
            'salaries_view' => $user->is_admin == 1 || in_array($user->id, [5263,9974,11533,5494,12787,13583,1739,2966,3794,3423,4143]),
            'quality_view' => $user->is_admin == 1 || in_array($user->id, [11533,5494,12787,13583,3460,3423,3794,3965,4143,5494,8257,9974,11208,3423]),
